...
FIXME Notes: keine Zeit für "was macht Nix, wie macht Nix was?", aber:
welche (Docker-)Probleme löst es, wann sollte/kann man es einsetzen?
...
➜ Trennung "Compile-Time" vs. Runtime (Multi-Stage Build)
...
- Kriegen Reproduzierbarkeit in den Griff: sowohl lokal als auch in
  Docker
- Minimalität: das minimale Docker-Image enthält die Anwendung + alle
...
FIXME Notes:
- habe Bauanleitungen für alles
- wenn ich weiß, was beteiligt ist, weiß ich, wenn sich ändert
...
FIXME Notes: Image-Größen vergleichen: go-backend, haskell-backend, both-backends
